...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH31",
  "title": "feat(gha): итеративный micro-loop в Coder — write lint fix commit",
  "description": "GitHub Issue #31: feat(gha): итеративный micro-loop в Coder — write lint fix commit\n\nУлучшить agents/coder.ts. Сервис: grandhub-agents.\n\nДобавить micro-loop внутри одной попытки вместо полного retry:\n1. LLM пишет код\n2. Запуск lint\n3. Если lint fail — LLM получает только ошибки и исправляет (не весь контекст заново)\n4. Запуск typecheck\n5. Если typecheck fail — LLM исправляет только TS ошибки\n6. Commit только если lint+typecheck прошли\nMax 3 micro-iterations. LLM prompt для fix: передать только список ошибок + текущий файл.\nallow_test_failure: true\nКритерии: micro-loop работает, ",
  "service": "api-gateway",
  "type": "feat",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #31 устранена",
  ...
  "timeout_minutes": 30,
  "cost_budget_usd": 2,
  "created_at": "2026-03-14T16:19:02.246Z",
  "github_issue": {
    "number": 31,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/31",
    "author": "Rivega42"
  }
