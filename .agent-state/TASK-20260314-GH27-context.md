...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH27",
  "title": "feat(api-gateway): middleware для логирования запросов",
  "description": "GitHub Issue #27: feat(api-gateway): middleware для логирования запросов\n\nДобавить middleware который логирует все входящие запросы.\n\n## Сервис\nservices/api-gateway/\n\n## Файлы\n- src/middleware/request-logger.ts — новый файл\n- src/index.ts — подключить middleware\n\n## Что сделать\n\nExpress middleware:\n1. Записывает в консоль: метод, путь, статус, время ответа (ms), userId если есть\n2. Для ошибочных запросов (4xx/5xx) — логирует body запроса и ответа\n3. Пропускает /health и /metrics (не логировать healthcheck шум)\n\nФормат лога (JSON):\n{ ts, method, path, status, duration_",
  "service": "api-gateway",
  "type": "feat",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #27 устранена",
  ...
  "github_issue": {
    "number": 27,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/27",
    "author": "Rivega42"
  }
