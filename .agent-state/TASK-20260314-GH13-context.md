...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH13",
  "title": "feat(skill-registry): поиск и фильтрация скиллов в marketplace API",
  "description": "GitHub Issue #13: feat(skill-registry): поиск и фильтрация скиллов в marketplace API\n\nУлучшить GET /api/v1/marketplace — добавить поиск и фильтрацию.\n\n## Сервис\nservices/skill-registry/\n\n## Файл\nsrc/routes/skills.ts — функция createMarketplaceRouter()\n\n## Что добавить\n\nРасширить GET /api/v1/marketplace чтобы принимал query params:\n- ?category=ANALYTICS — фильтр по категории (SkillCategory enum)\n- ?tag=анализ — фильтр по тегу (contains в массиве tags)\n- ?q=конкурент — полнотекстовый поиск по name и description (icontains)\n- ?level=1 — фильтр по уровню\n- ?limit=10&offset=0 — пагина",
  "service": "skill-registry",
  "type": "feat",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #13 устранена",
  ...
  "timeout_minutes": 30,
  "cost_budget_usd": 2,
  "created_at": "2026-03-14T13:06:10.074Z",
  "github_issue": {
    "number": 13,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/13",
    "author": "Rivega42"
  }
...
import { createSkillRouter, createMarketplaceRouter, createAgentSkillRouter } from './routes/skills.js';
...
export function createApp(logger: Logger): express.Application {
  ...
  // Health check (3.5) — checks real dependencies
...
export function createMarketplaceRouter(): Router {
  ...
  router.get('/', async (req, res, next) => {
    try {
      const { tag, level, price_model, q, limit = '20', offset = '0' } = req.query as Record<string, string>;
      const where: Record<string, unknown> = { status: 'PUBLISHED', isPublic: true };
      if (tag) where['tags'] = { has: tag };
      if (level !== undefined) where['level'] = parseInt(level);
      if (price_model) where['priceModel'] = price_model;
      if (q) where['name'] = { contains: q, mode: 'insensitive' };

      const [skills, total] = await Promise.all([
        db().skill.findMany({ where, take: parseInt(limit), skip: parseInt(offset), orderBy: { downloadCount: 'desc' } }),
        db().skill.count({ where }),
      ]);
      res.json({ success: true, data: { skills, total } });
    } catch (err) { next(err); }
  });

  // POST /api/v1/marketplace/:id/install
  router.post('/:id/install', async (req: any, res, next) => {
    try {
      const { id } = req.params;
      ...
      await db().skill.update({ where: { id }, data: { downloadCount: { increment: 1 } } });
  ...
  router.post('/:id/publish', async (req: any, res, next) => {
    try {
      ...
      if (!skill) { res.status(404).json({ error: 'Skill not found or not owned' }); return; }
  ...
  router.get('/my', async (req: any, res, next) => {
    try {
      const userId = req.user?.id;
      if (!userId) { res.status(401).json({ error: 'Unauthorized' }); return; }
      const skills = await db().skill.findMany({ where: { authorId: userId }, orderBy: { createdAt: 'desc' } });
      res.json({ success: true, data: skills });
    } catch (err) { next(err); }
  });

  // GET /api/v1/marketplace/developer/stats
  router.get('/developer/stats', async (req: any, res, next) => {
    try {
      const userId = req.user?.id;
      if (!userId) { res.status(401).json({ error: 'Unauthorized' }); return; }
      const skills = await db().skill.findMany({
        where: { authorId: userId },
        select: { id: true, name: true, downloadCount: true, ratingAvg: true, priceModel: true, priceCents: true }
      });
      const totalInstalls = skills.reduce((s: number, sk: any) => s + sk.downloadCount, 0);
      res.json({ success: true, data: { skills, totalInstalls, skillCount: skills.length } });
    } catch (err) { next(err); }
  });
