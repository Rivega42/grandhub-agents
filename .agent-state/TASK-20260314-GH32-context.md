...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH32",
  "title": "fix(gha): сбрасывать running задачи в pending при старте — queue.json recovery",
  "description": "GitHub Issue #32: fix(gha): сбрасывать running задачи в pending при старте — queue.json recovery\n\nФайл: /opt/grandhub-agents/agents/orchestrator.ts\n\nДобавить в самое начало функции main() или сразу после загрузки queue (после строки где читается queue.json через fs.readFileSync или loadQueue):\n\nНайти место где queue загружается из файла queue.json и добавить сразу после:\n\nfor (const entry of queue) {\n  if (entry.status === pending_string_but_as_running) {\n    entry.status = pending_as_string;\n    delete (entry as any).started_at;\n    console.error([orchestrator] Recovery: сбрасываю зависшую ",
  "service": "api-gateway",
  "type": "fix",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #32 устранена",
  ...
  "timeout_minutes": 30,
  "cost_budget_usd": 2,
  "created_at": "2026-03-14T16:27:32.656Z",
  "github_issue": {
    "number": 32,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/32",
    "author": "Rivega42"
  }
