...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH9",
  "title": "feat(web): страница idea.grandhub.ru — голосование за скиллы",
  "description": "GitHub Issue #9: feat(web): страница idea.grandhub.ru — голосование за скиллы\n\nПубличная страница для подачи и голосования за идеи новых скиллов.\n\n## Что нужно\n\nСтраница /ideas (или отдельный поддомен) в Next.js:\n\n1. Лента идей (карточки): название, описание, голоса, статус\n2. Кнопка Проголосовать (авторизация через Telegram Login Widget)\n3. Форма Предложить идею: title + description + category\n4. Статусы идеи: new → approved → in_progress → completed\n5. Порог автоматического запуска: 50 голосов ��� создаёт GitHub issue в grandhub-feedback\n\n## БД\n\n```sql\nCREATE TABLE skill",
  "service": "telegram-bot",
  "type": "feat",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #9 устранена",
  ...
  "timeout_minutes": 30,
  "cost_budget_usd": 2,
  "created_at": "2026-03-14T09:58:08.141Z",
  "github_issue": {
    "number": 9,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/9",
    "author": "Rivega42"
  }
  ...
  internalApp.post('/api/internal/send', async (req: Request, res: Response) => {
    try {
      ...
      if (!telegramId || !message) {
        res.status(400).json({ error: 'Missing telegramId or message' });
        return;
      }
...
export async function handleSiteBuilderCallback(ctx: BotContext, action: string) {
  ...
  if (action === 'site_confirm' && state) {
    ...
    try {
      ...
      await ctx.reply('❌ Не удалось создать сайт. Попробуйте позже или обратитесь в поддержку.');
      siteBuilderStates.delete(userId);
    }
...
export async function handleSubscriptionMessage(ctx: BotContext) {
  ...
  const cycleText = cycleTextMap[parsed.billingCycle] ?? 'в месяц';
...
export async function handleSubscriptionStatsCommand(ctx: BotContext) {
  ...
  try {
    ...
    if (stats.byCategory && Object.keys(stats.byCategory).length > 0) {
      ...
      const sorted = Object.entries(stats.byCategory as Record<string, number>).sort(
        ([, a], [, b]) => b - a,
      );
...
export async function handleSubscriptionCallbacks(ctx: BotContext) {
  ...
  if (data === 'sub_stats') {
    ...
    try {
      ...
      await axios.post(`${API_URL}/subscriptions`, parsed, {
        headers: {
...
export function isSubscriptionIdeaTrigger(text: string): boolean {
  ...
  return (
    subscriptionKeywords.some((kw) => lowerText.includes(kw)) &&
...
async function handleProjectDeleteConfirm(ctx: BotContext, projectId: string) {
  ...
  await ctx.editMessageText('⚠️ *Удалить проект?*\n\nЭто действие нельзя отменить.', {
    parse_mode: 'Markdown',
    reply_markup: keyboard,
  });
  await ctx.answerCallbackQuery();
  ...
  try {
    ...
    if (similarIdeas.length > 0) {
      ...
      for (const idea of similarIdeas) {
        ...
        keyboard.text(`👍 ${idea.title.slice(0, 40)}... (${similarity}%)`, `vote_idea_${idea.id}`);
        keyboard.row();
      }
...
async function handleIdeaCollectionStep(ctx: BotContext, userId: number, messageText: string) {
  ...
  switch (state.stage) {
    case 'problem':
      ...
      await ctx.reply(
        '👥 Для кого это будет полезно? (например: студенты, предприниматели, все пользователи)',
      );
      break;
...
Хочешь больше навыков? → /plan`;
    ...
    if (data.startsWith('toggle_skill_')) {
      ...
      const currentSkills = (ctx.user.skills as UserSkills | undefined) ?? defaultSkills;
      const updatedSkills: UserSkills = { ...currentSkills };
      ...
      (ctx.user as { skills?: UserSkills }).skills = updatedSkills;
...
import { BotContext } from '../middleware/auth';
import { texts } from '../config/texts';
import { getOnboardingSkillsKeyboard } from '../keyboards/skills';
import type { UserSkills } from '../keyboards/skills';
import { apiClient } from '../services/api-client';
...
export async function handleOnboardingCallback(ctx: BotContext, action: string) {
  try {
    ...
    if (action.startsWith('onb_skill_')) {
      ...
      await apiClient.updateUser(ctx.user.id, {
        skills: skills as import('../services/api-client').UserSkills,
      });
...
function getDaysWord(count: number): string {
  ...
  const index = count % 100 > 4 && count % 100 < 20 ? 2 : (cases[caseIndex] ?? 2);
