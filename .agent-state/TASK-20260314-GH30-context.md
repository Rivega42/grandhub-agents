...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH30",
  "title": "feat(gha): Tester Agent — анализирует провалы тестов через LLM",
  "description": "GitHub Issue #30: feat(gha): Tester Agent — анализирует провалы тестов через LLM\n\nСоздать agents/tester.ts. Сервис: grandhub-agents.\n\nЧто делает:\n1. Принимает --task-id и --service-path\n2. Запускает bash scripts/test.sh в worktree\n3. Если failures — LLM анализирует что сломалось и предлагает fix\n4. Возв��ащает JSON: passed boolean, failures array, suggestions array\n\nLLM: claude-opus-4-6 через localhost:18789, system QA-инженер.\nЗапуск: ts-node agents/tester.ts --task-id TASK-123 --service-path /opt/grandhub-v3/services/telegram-bot\nallow_test_failure: true\nКритерии: компилиру",
  "service": "telegram-bot",
  "type": "feat",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #30 устранена",
  ...
  "github_issue": {
    "number": 30,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/30",
    "author": "Rivega42"
  }
