...
\n# TASK SPEC\n{
  "task_id": "TASK-20260314-GH3",
  "title": "fix(infra): отключить устаревшие grandbazar-* systemd юниты",
  "description": "GitHub Issue #3: fix(infra): отключить устаревшие grandbazar-* systemd юниты\n\nНесколько systemd юнитов постоянно падают (auto-restart loop) потому что указывают на несуществующие пути `/opt/grandbazar/`.\n\n## Падающие юниты\n\n- `grandbazar-admin-api.service` — нет .env, нет пути\n- `grandbazar-habits.service` — нет рабочей директории  \n- `grandbazar-ideas.service` — нет рабочей директории\n- `grandbazar-api.service` — нет пути\n\n## Задача\n\nДля каждого юнита — проверить нужен ли он. Если нет — `systemctl disable --now <unit>`.\nЕсли нужен — обновить `WorkingDirectory` и `Environ",
  "service": "api-gateway",
  "type": "fix",
  "priority": "medium",
  "file_scope": [],
  "acceptance_criteria": [
    "Проблема из issue #3 устранена",
  ...
  "timeout_minutes": 30,
  "cost_budget_usd": 2,
  "created_at": "2026-03-14T09:14:35.663Z",
  "github_issue": {
    "number": 3,
    "url": "https://github.com/Rivega42/grandhub-feedback/issues/3",
    "author": "Rivega42"
  }
